...
export async function createPatient(formData) {
  ...
  await transaction(async (client) => {
    ...
    await client.query(
      ...
      [crypto.randomUUID(), userId, text(formData, "occupation"), number(formData, "numChildren"), text(formData, "socialSecurityType"), text(formData, "phoneNumber"), text(formData, "region"), text(formData, "address"), text(formData, "postalCode"), createdAt]
    );
    await client.query(
    ...
    const phones = list(formData, "emergencyPhone");
...
export async function updateContactAndSos(userId, formData) {
  ...
  await query(
    ...
    [userId, text(formData, "occupation"), number(formData, "numChildren"), text(formData, "socialSecurityType"), text(formData, "phoneNumber"), text(formData, "region"), text(formData, "address"), text(formData, "postalCode"), updatedAt]
  );
...
export async function addAppointment(userId, formData) {
  await query(
    ...
    [crypto.randomUUID(), userId, text(formData, "doctorName"), text(formData, "specialty"), text(formData, "appointmentDate") || null, text(formData, "clinicLocation"), text(formData, "phoneContact"), now()]
  );
}
