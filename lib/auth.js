...
function verifySignedBody(token) {
...
export function verifyPortalToken(token) {
  const payload = verifySignedBody(token);
  if (!payload || payload.role !== "PORTAL" || !payload.userId) return null;
